...
:: --- Always copy static assets and public folder ---
echo  [SETUP] Copying static assets...
...
set "_DB=%~dp0.next\standalone\prisma\dev.db"
set "_DB=%_DB:\=/%"
set "DATABASE_URL=file:%_DB%"

:: --- Verify static files ---
if not exist ".next\standalone\.next\static" (
    echo  [ERROR] Static files missing in standalone! Build may have failed.
    pause
    exit /b 1
)
...
:: --- Run server with absolute DATABASE_URL ---
...
:: --- Always copy static assets (needed after every build) ---
echo  [SETUP] Copying static assets into standalone...
xcopy /E /I /Y /Q ".next\static" ".next\standalone\.next\static" > nul 2>&1
xcopy /E /I /Y /Q "public"       ".next\standalone\public"       > nul 2>&1
...
if not exist ".next\standalone\.next\static" (
    echo  [ERROR] Static files missing! Check that npm run build completed.
    pause
    exit /b 1
)
...
:: Run node DIRECTLY (no pipe). Clean kill when window closes.
node server.js
