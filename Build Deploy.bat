@echo off
title Atelier Moein - Build Deploy Package
chcp 65001 > nul
cd /d "%~dp0"

echo.
echo  ========================================
echo   Building Deployment Package...
echo  ========================================
echo.

:: --- Step 1: Build ---
echo  [1/5] Building Next.js production bundle...
call npm run build
if errorlevel 1 (
    echo  [ERROR] Build failed.
    pause
    exit /b 1
)

:: --- Step 2: Clean and copy standalone ---
echo  [2/5] Preparing deploy folder...
if exist "deploy" rmdir /s /q "deploy"
xcopy /E /I /Q ".next\standalone" "deploy" > nul 2>&1

:: --- Step 3: Copy static assets directly into deploy ---
echo  [3/5] Copying static assets...
xcopy /E /I /Q ".next\static" "deploy\.next\static" > nul 2>&1
xcopy /E /I /Q "public"       "deploy\public"        > nul 2>&1

:: --- Step 4: Copy database ---
echo  [4/5] Copying database...
if not exist "prisma\prisma\dev.db" (
    echo  [ERROR] Database not found at prisma\prisma\dev.db
    echo  [ERROR] Run start.bat first to create the database.
    pause
    exit /b 1
)
if not exist "deploy\prisma" mkdir "deploy\prisma"
copy /Y "prisma\prisma\dev.db" "deploy\prisma\dev.db" > nul 2>&1

:: --- Step 5: Create Start.bat inside deploy ---
echo  [5/5] Creating launcher...
(
    echo @echo off
    echo title Atelier Moein - Production Server
    echo chcp 65001 ^> nul
    echo cd /d "%%~dp0"
    echo.
    echo :: Kill previous server on port 3000
    echo for /f "tokens=5" %%%%p in ^('netstat -ano ^^^| findstr ":3000 " ^^^| findstr "LISTENING"'^) do ^(
    echo     taskkill /F /PID %%%%p ^> nul 2^>^&1
    echo ^)
    echo.
    echo :: Build ABSOLUTE path for DATABASE_URL
    echo :: Prisma SQLite on Windows requires "file:C:/path" format
    echo set "_DB=%%~dp0prisma\dev.db"
    echo set "_DB=%%_DB:\=//%%"
    echo set "DATABASE_URL=file:%%_DB%%"
    echo.
    echo set "LOCAL_IP=localhost"
    echo for /f "tokens=2 delims=:" %%%%a in ^('ipconfig ^^^| findstr /c:"IPv4"'^) do ^(
    echo     set "LOCAL_IP=%%%%a"
    echo     goto :got_ip
    echo ^)
    echo :got_ip
    echo set "LOCAL_IP=%%LOCAL_IP: =%%"
    echo.
    echo echo.
    echo echo  ========================================
    echo echo   Atelier Moein - Production Server
    echo echo  ========================================
    echo echo.
    echo echo   Local:   http://localhost:3000
    echo echo   Network: http://%%LOCAL_IP%%:3000
    echo echo.
    echo echo   To stop: close this window or Ctrl+C
    echo echo.
    echo.
    echo set NODE_ENV=production
    echo set PORT=3000
    echo set COOKIE_SECURE=false
    echo node server.js
    echo if errorlevel 1 pause
) > "deploy\Start.bat"

:: --- Verify output ---
if not exist "deploy\.next\static" (
    echo  [ERROR] Static files missing from deploy!
    pause
    exit /b 1
)
if not exist "deploy\prisma\dev.db" (
    echo  [ERROR] Database missing from deploy!
    pause
    exit /b 1
)

echo.
echo  ========================================
echo   Deploy package ready at: .\deploy\
echo  ========================================
echo.
echo   Verified contents:
for %%f in ("deploy\.next\static" "deploy\public" "deploy\prisma\dev.db" "deploy\Start.bat" "deploy\server.js") do (
    if exist "%%~f" (echo     [OK] %%~f) else (echo     [MISSING] %%~f)
)
echo.
echo   To use on another machine:
echo     1. Copy the deploy\ folder to the target machine
echo     2. Make sure Node.js is installed
echo     3. Double-click Start.bat
echo.
pause
@echo off
title Atelier Moein - Build Deploy Package
chcp 65001 > nul
cd /d "%~dp0"

echo.
echo  ========================================
echo   Building Deployment Package...
echo  ========================================
echo.

:: --- Step 1: Build ---
echo  [1/5] Building Next.js production bundle...
call npm run build
if errorlevel 1 (
    echo  [ERROR] Build failed.
    pause
    exit /b 1
)

:: --- Step 2: Clean and copy standalone ---
echo  [2/5] Preparing deploy folder...
if exist "deploy" rmdir /s /q "deploy"
xcopy /E /I /Q ".next\standalone" "deploy" > nul 2>&1

:: --- Step 3: Copy static assets DIRECTLY into deploy (not via standalone) ---
echo  [3/5] Copying static assets...
xcopy /E /I /Q ".next\static" "deploy\.next\static" > nul 2>&1
xcopy /E /I /Q "public"       "deploy\public"        > nul 2>&1

:: --- Step 4: Copy database ---
echo  [4/5] Copying database...
if not exist "prisma\prisma\dev.db" (
    echo  [ERROR] Database not found at prisma\prisma\dev.db
    echo  [ERROR] Run start.bat first to create the database.
    pause
    exit /b 1
)
if not exist "deploy\prisma" mkdir "deploy\prisma"
copy /Y "prisma\prisma\dev.db" "deploy\prisma\dev.db" > nul 2>&1

:: --- Step 5: Create Start.bat inside deploy ---
echo  [5/5] Creating launcher...
(
    echo @echo off
    echo title Atelier Moein - Production Server
    echo chcp 65001 ^> nul
    echo cd /d "%%~dp0"
    echo.
    echo :: Kill previous server on port 3000
    echo for /f "tokens=5" %%%%p in ^('netstat -ano ^^^| findstr ":3000 " ^^^| findstr "LISTENING"'^) do ^(
    echo     taskkill /F /PID %%%%p ^> nul 2^>^&1
    echo ^)
    echo.
    echo set "LOCAL_IP=localhost"
    echo for /f "tokens=2 delims=:" %%%%a in ^('ipconfig ^^^| findstr /c:"IPv4"'^) do ^(
    echo     set "LOCAL_IP=%%%%a"
    echo     goto :got_ip
    echo ^)
    echo :got_ip
    echo set "LOCAL_IP=%%LOCAL_IP: =%%"
    echo.
    echo echo.
    echo echo  ========================================
    echo echo   Atelier Moein - Production Server
    echo echo  ========================================
    echo echo.
    echo echo   Local:   http://localhost:3000
    echo echo   Network: http://%%LOCAL_IP%%:3000
    echo echo.
    echo echo   To stop: close this window or Ctrl+C
    echo echo.
    echo.
    echo set NODE_ENV=production
    echo set PORT=3000
    echo set DATABASE_URL=file:./prisma/dev.db
    echo set COOKIE_SECURE=false
    echo node server.js
    echo if errorlevel 1 pause
) > "deploy\Start.bat"

:: --- Verify ---
if not exist "deploy\.next\static" (
    echo  [ERROR] Static files missing from deploy!
    pause
    exit /b 1
)
if not exist "deploy\prisma\dev.db" (
    echo  [ERROR] Database missing from deploy!
    pause
    exit /b 1
)

echo.
echo  ========================================
echo   Deploy package ready at: .\deploy\
echo  ========================================
echo.
echo   Verified:
for %%f in ("deploy\.next\static" "deploy\public" "deploy\prisma\dev.db" "deploy\Start.bat" "deploy\server.js") do (
    if exist "%%~f" (echo     OK: %%~f) else (echo     MISSING: %%~f)
)
echo.
echo   To deploy on another machine:
echo     1. Copy the deploy\ folder
echo     2. Make sure Node.js is installed
echo     3. Double-click Start.bat
echo.
pause
@echo off
title Atelier Moein - Build Deploy Package
chcp 65001 > nul
cd /d "%~dp0"

echo.
echo  ========================================
echo   Building Deployment Package...
echo  ========================================
echo.

:: Step 1 - Build
echo  [1/5] Building Next.js production bundle...
call npm run build
if errorlevel 1 (
    echo  [ERROR] Build failed.
    pause
    exit /b 1
)

:: Step 2 - Prepare deploy folder
echo  [2/5] Preparing deploy folder...
if exist "deploy" rmdir /s /q "deploy"
xcopy /E /I /Q ".next\standalone" "deploy" > nul 2>&1

:: Step 3 - Copy static assets
echo  [3/5] Copying static assets...
xcopy /E /I /Q ".next\static" "deploy\.next\static" > nul 2>&1
xcopy /E /I /Q "public"       "deploy\public"       > nul 2>&1

:: Step 4 - Copy database
echo  [4/5] Copying database...
if not exist "deploy\prisma" mkdir "deploy\prisma"
copy /Y "prisma\prisma\dev.db" "deploy\prisma\dev.db" > nul 2>&1

:: Step 5 - Create Start.bat inside deploy/
echo  [5/5] Creating launcher...

> "deploy\Start.bat" (
echo @echo off
echo title Atelier Moein - Production Server
echo chcp 65001 ^> nul
echo cd /d "%%~dp0"
echo.
echo :: Kill previous server on port 3000
echo for /f "tokens=5" %%%%p in ^('netstat -ano ^^^| findstr ":3000 " ^^^| findstr "LISTENING"'^) do ^(
echo     taskkill /F /PID %%%%p ^> nul 2^>^&1
echo ^)
echo.
echo set "LOCAL_IP=localhost"
echo for /f "tokens=2 delims=:" %%%%a in ^('ipconfig ^^^| findstr /c:"IPv4"'^) do ^(
echo     set "LOCAL_IP=%%%%a"
echo     goto :got_ip
echo ^)
echo :got_ip
echo set "LOCAL_IP=%%LOCAL_IP: =%%"
echo.
echo echo  ========================================
echo echo   Atelier Moein - Production Server
echo echo  ========================================
echo echo.
echo echo   Local:   http://localhost:3000
echo echo   Network: http://%%LOCAL_IP%%:3000
echo echo.
echo echo   To stop: close this window or Ctrl+C
echo echo.
echo.
echo set NODE_ENV=production
echo set PORT=3000
echo set DATABASE_URL=file:./prisma/dev.db
echo set COOKIE_SECURE=false
echo node server.js
echo if errorlevel 1 pause
)

echo.
echo  ========================================
echo   Deploy package ready at: .\deploy\
echo  ========================================
echo.
echo   To deploy on another machine:
echo     1. Copy the deploy\ folder
echo     2. Make sure Node.js is installed
echo     3. Double-click Start.bat
echo.
pause
@echo off
title Atelier Moein - Build Deploy Package
chcp 65001 > nul
cd /d "%~dp0"

echo.
echo  ========================================
echo   Building Deployment Package...
echo  ========================================
echo.

:: Step 1 - Build
echo [1/5] Building Next.js production bundle...
call npm run build
if errorlevel 1 (
    echo  [ERROR] Build failed.
    pause & exit /b 1
)

:: Step 2 - Prepare deploy folder
echo [2/5] Preparing deploy folder...
if exist "deploy" rmdir /s /q "deploy"
xcopy /E /I /Q ".next\standalone" "deploy" > nul 2>&1

:: Step 3 - Copy static assets
echo [3/5] Copying static assets...
xcopy /E /I /Q ".next\static" "deploy\.next\static" > nul 2>&1
xcopy /E /I /Q "public"       "deploy\public"       > nul 2>&1

:: Step 4 - Copy database
echo [4/5] Copying database...
if not exist "deploy\prisma" mkdir "deploy\prisma"
copy /Y "prisma\prisma\dev.db" "deploy\prisma\dev.db" > nul 2>&1

:: Step 5 - Add launcher scripts inside deploy/
echo [5/5] Creating launchers...

(
echo @echo off
echo title Atelier Moein
echo chcp 65001 ^> nul
echo cd /d "%%~dp0"
echo.
echo for /f "tokens=2 delims=:" %%%%a in ^('ipconfig ^| findstr /c:"IPv4"'^) do ^(
echo     set "LOCAL_IP=%%%%a"
echo     goto :got_ip
echo ^)
echo :got_ip
echo set "LOCAL_IP=%%LOCAL_IP: =%%"
echo.
echo if not exist "logs" mkdir "logs"
echo set LOGFILE=%%~dp0logs\server.log
echo.
echo echo  ========================================
echo echo   Atelier Moein ^- Production Server
echo echo  ========================================
echo echo.
echo echo   Local:   http://localhost:3000
echo echo   Network: http://%%LOCAL_IP%%:3000
echo echo   Logs:    %%LOGFILE%%
echo echo.
echo echo   Press Ctrl+C to stop...
echo echo.
echo set NODE_ENV=production
echo set PORT=3000
echo set DATABASE_URL=file:./prisma/dev.db
echo set COOKIE_SECURE=false
echo node server.js 2^>^&1 ^| powershell -NoProfile -Command "$input ^| Tee-Object -FilePath '%%LOGFILE%%' -Append"
echo if errorlevel 1 pause
) > "deploy\Start.bat"

echo.
echo  ========================================
echo   Deploy package ready at: .\deploy\
echo  ========================================
echo.
echo   To deploy on another machine:
echo     1. Copy the deploy\ folder
echo     2. Make sure Node.js is installed
echo     3. Double-click Start.bat
echo.
pause
